...
object NetworkTraffic {
  def main(args: Array[String]): Unit = {
    ...
    //数据
    val stream = env.readTextFile("I:\\idea\\UserBehaviorAnalysis\\NetworkTrafficAnalysis1\\src\\main\\resources\\apache.log")
      .map(lien=>{
        val lienarray = lien.split(" ")
        val simpleDateFormat = new SimpleDateFormat("dd/MM/yyyy:HH:mm:ss")
        val timestamp = simpleDateFormat.parse(lienarray(3)).getTime
        ApacheLogEvent(lienarray(0),lienarray(1),timestamp,lienarray(5),lienarray(6))
      })
    //创建时间戳和水位线
